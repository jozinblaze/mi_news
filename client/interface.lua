...
----    newspaper options    ----
lib.registerContext({
    id = 'newspaper_options',
    title = 'Purchase Newspaper',
    options = {
      {
        title = 'The Daily Globe',
        description = 'Spinning the latest news',
        icon = 'globe',
        onSelect = function()
            print('purchased paper')
        end,
        metadata = {
            {label = 'cost', value = '$2'},
          },
      ...
      {
        title = 'The Los Santos Meteor',
        description = 'Los Santos\' most widely read newspaper',
        icon = 'meteor',
        onSelect = function()
            print('purchased paper')
        end,
...
CreateStory = function(data)
    local input = lib.inputDialog('Create News Article', {
    {type = 'input', label = 'Article Title', description = 'Something catchy', required = true, min = 4, max = 20},
...
----    load global object targets    ----
Citizen.CreateThread(function()
    exports.ox_target:addModel(Data.stands, {
        {
            name = 'getpaper',
            icon = 'fa-solid fa-file-invoice-dollar',
            label = 'Purchase Newspaper',
            canInteract = function(_, distance)
                return distance < 1.5
            end,
            onSelect = function()
                lib.showContext('newspaper_options')
            end
        },
    })
end)
